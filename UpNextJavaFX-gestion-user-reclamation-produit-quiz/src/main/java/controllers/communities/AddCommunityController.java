...
public class AddCommunityController {
    ...
    private void handleCreateImage() {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/communities/generate-image-dialog.fxml"));
    ...
    private void handleCreateCommunity() {
        ...
        if (newCommunity != null) {
            ...
            dialogRoot.getStylesheets().add(getClass().getResource("/styles.css").toExternalForm());
            ...
            // Navigate back to the appropriate page based on session type
            try {
                // Check if the user is an artist
                SessionType currentSession = SessionManager.getInstance().getSessionType();
                if (currentSession == SessionType.ARTIST) {
                    // For artists, navigate to user-communities-view
                    SceneTransitionUtil.changeContentWithPreload(
                        ...
                        controller -> controller.loadCommunities() // Load communities for the view
                    );
                } else {
                    // For other users, navigate to the main page
                    SceneTransitionUtil.changeContentWithPreload(
